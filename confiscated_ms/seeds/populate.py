...
for data in deliverydata[:20]:
    r = requests.post(url = "http://92.168.99.102:8000/confiscated_ms/delivery/", data = data) 
    print('Delivery Post Status:',r)

for data in categorydata[:20]:
    r = requests.post(url = "http://192.168.99.102:8000/confiscated_ms/category/", data = data) 
    print('Category Post Status:',r)
    
for data in itemdata[:80]:
    r = requests.post(url = "http://192.168.99.102:8000/confiscated_ms/items/", data = data) 
